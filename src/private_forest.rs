...
use wnfs::{libipld::Cid, private::{RevisionRef, AesKey}, common::Metadata};
use rand::{thread_rng, rngs::ThreadRng};
use wnfs::{private::{PrivateForest, PrivateRef, PrivateDirectory, PrivateNode}, hamt::Hasher, namefilter::Namefilter, common::{utils, BlockStore}};
use std::{
    rc::Rc, 
    fs::{File}, 
    io::{Read, Write},
...
impl<'a> PrivateDirectoryHelper<'a> {
    ...
    pub async fn create_private_forest(&mut self) -> Result<Cid, String> {
        ...
        if private_root_cid.is_ok(){
            ...
            trace!("wnfsError occured in create_private_forest: {:?}", private_root_cid.as_ref().err().unwrap());
    ...
    pub async fn get_private_ref(&mut self, wnfs_key: Vec<u8>, forest_cid: Cid) -> Result<PrivateRef, String> {
        ...
        let reloaded_revision_ref = RevisionRef::with_seed(Namefilter::default(), ratchet_seed, inumber);
        ...
        if forest.is_ok() {
            ...
            if fetched_node.is_ok() {
                let tmp_node = fetched_node.ok().unwrap()
                    .search_latest(&forest_unwrapped, &mut self.store).await;
                if tmp_node.is_ok() {
                    ...
                    if latest_dir_res.is_ok() {
                        ...
                        // TODO: function is removed
                        // let private_ref = latest_dir.header.get_private_ref();

                        // Ok(private_ref)
                        // TODO: is this reasonable?
                        Ok(PrivateRef { saturated_name_hash: latest_dir.header.get_saturated_name_hash()
                            , temporal_key: latest_dir.header.derive_temporal_key(), 
                            content_cid: forest_cid})
                    } else {
    ...
    pub async fn read_filestream_to_path(&mut self, local_filename: &String, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String], index: usize) -> Result<bool, String> {
    ...
    pub async fn read_file_to_path(&mut self, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String], filename: &String) -> Result<String, String> {
    ...
    pub async fn read_file(&mut self, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String]) -> Result<Vec<u8>, String> {
        let res = root_dir
    ...
    pub async fn ls_files(&mut self, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String]) -> Result<Vec<(String, Metadata)>, String> {
...
impl<'a> PrivateDirectoryHelper<'a> {
    ...
    pub fn synced_read_file_to_path(&mut self, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String], filename: &String) -> Result<String, String>
    {
        let runtime =
            tokio::runtime::Runtime::new().expect("Unable to create a runtime");
    ...
    pub fn synced_read_file(&mut self, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String]) -> Result<Vec<u8>, String>
    {
        let runtime =
            tokio::runtime::Runtime::new().expect("Unable to create a runtime");
    ...
    pub fn synced_read_filestream_to_path(&mut self, local_filename: &String, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String], index: usize) -> Result<bool, String> {
        let runtime =
        tokio::runtime::Runtime::new().expect("Unable to create a runtime");
    ...
    pub fn synced_ls_files(&mut self, forest: &mut Rc<PrivateForest>, root_dir: &mut Rc<PrivateDirectory>, path_segments: &[String]) -> Result<Vec<(String, Metadata)>, String>
    {
        let runtime =
            tokio::runtime::Runtime::new().expect("Unable to create a runtime");
