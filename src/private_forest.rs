...
use wnfs::{
    common::{utils, BlockStore, Metadata, CODEC_RAW, CODEC_DAG_CBOR},
    hamt::Hasher,
...
impl<'a> PrivateDirectoryHelper<'a> {
    ...
    pub async fn load_with_wnfs_key(
        ...
        trace!("wnfsInfo: load_with_wnfs_key started");
        let rng = &mut thread_rng();
        let root_did: String;
        let seed: [u8; 32];
        if wnfs_key.is_empty() {
            ...
            if forest_res.is_ok() {
                ...
                if counter_res.is_ok() {
                    ...
                    if node_res.is_ok() {
                        ...
                        let latest_node_res = node.search_latest(forest, store).await;
                        
                        if latest_node_res.is_ok() {
                            let latest_node = latest_node_res.ok().unwrap();
                            let header = latest_node.get_header();
                            trace!("wnfsInfo header of latest node in reload: {:?}",header);
                            let latest_root_dir_res = latest_node.as_dir();
                            if latest_root_dir_res.is_ok() {
                                unsafe {
                                    STATE.lock().unwrap().update(true, wnfs_key.to_owned());
                                }
                                let latest_root_dir = latest_root_dir_res.ok().unwrap();
                                let root_metadata = latest_root_dir.get_metadata();
                                trace!("wnfsInfo metadata of latest root in reload: {:?}",root_metadata);

                                Ok(Self {
                                    store: store.to_owned(),
                                    forest: forest.to_owned(),
                                    root_dir: latest_root_dir,
                                    rng: rng.to_owned(),
                                })
                            } else {
                                trace!(
                                    "wnfsError in load_with_wnfs_key: {:?}",
                                    latest_root_dir_res.as_ref().err().unwrap().to_string()
                                );
                                Err(latest_root_dir_res.err().unwrap().to_string())
                            }
                        } else {
                            trace!(
                                "wnfsError occured in load_with_wnfs_key: {:?}",
                                latest_node_res.as_ref().to_owned().err().unwrap().to_string()
                            );
                            Err(latest_node_res.as_ref().to_owned().err().unwrap().to_string())
                        }
                    } else {
    ...
    fn get_file_as_byte_vec(&mut self, filename: &String) -> Result<(Vec<u8>, i64), String> {
        let mut file = File::open(&filename).map_err(|e| {
            trace!("wnfsError in get_file_as_byte_vec, no file found: {:?}", e);
            "wnfsError no file found".to_string()
        })?;
    
        let metadata = std::fs::metadata(&filename).map_err(|e| {
            trace!("wnfsError in get_file_as_byte_vec, unable to read metadata: {:?}", e);
            "wnfsError unable to read metadata".to_string()
        })?;
        
        let modification_time_seconds = metadata.mtime();
        
        let mut buffer = Vec::new(); // No need to pre-allocate with the file's length
        file.read_to_end(&mut buffer).map_err(|e| {
            trace!("wnfsError in get_file_as_byte_vec, unable to read file: {:?}", e);
            "wnfsError unable to read file".to_string()
        })?;
    ...
    pub async fn write_file_stream_from_path(
        ...
        if let Ok(file) = filedata {
            ...
            let writefile_res = self
                .write_file_stream(path_segments, &mut reader, modification_time_seconds).await;
            sleep(Duration::from_secs(2)).await;
            return writefile_res;
        } else {
    ...
    pub async fn read_filestream_to_path(
        ...
        if let Ok(mut local_file_handler) = local_file {
                ...
                if result.is_some() {
                    ...
                    if is_file {
                        ...
                        if file_res.is_ok() {
                            ...
                            trace!(
                                "wnfsError1 occured in read_filestream_to_path on file_res: {:?}",
                                file_res.as_ref().err().unwrap().to_string()
                            );
                            Err(file_res.err().unwrap().to_string())
                        }
                    } else {
                        trace!("wnfsError1 occured in read_filestream_to_path on is_file");
                        Err("wnfsError1 occured in read_filestream_to_path on is_file".to_string())
                    }
                } else {
                    trace!("wnfsError1 occured in read_filestream_to_path on result");
                    Err("wnfsError1 occured in read_filestream_to_path on result".to_string())
                }
            } else {
                trace!(
                    "wnfsError occured in read_filestream_to_path on private_node_result: {:?} for path_segments: {:?}",
            ...
            trace!(
                "wnfsError1 occured in read_filestream_to_path on local_file {:?}",
    ...
    pub async fn put(store: &mut FFIFriendlyBlockStore<'a>, key: Vec<u8>, value: Vec<u8>) {
        store.ffi_store.put_block(key, value).unwrap();
    }

    pub async fn get(store: &mut FFIFriendlyBlockStore<'a>, key: Vec<u8>) {
        store.ffi_store.get_block(key).unwrap();
    }
}
...
impl<'a> PrivateDirectoryHelper<'a> {
    pub fn synced_init(
        ...
        let store_tmp = KVBlockStore::new(String::from("/data/user/0/land.fx.app/cache/tmp/datastore"), CODEC_DAG_CBOR);
        
        trace!("########################AppMock: synced_init##########################");
        let size_res = store_tmp.store.size_on_disk();
        let buckets = store_tmp.store.buckets();
        
        let blockstore = &mut FFIFriendlyBlockStore::new(Box::new(store_tmp));

        if size_res.is_err() {
            trace!("##### AppMock: Error happened #######: {:?}",size_res.err() );
        } else {
            trace!("##### AppMock: size on disk #######: {:?}",size_res.ok().unwrap() );
        }
        
        trace!("##### AppMock: buckets #######: {:?}",buckets );

        let runtime = tokio::runtime::Runtime::new().expect("Unable to create a runtime");
        return runtime.block_on(PrivateDirectoryHelper::init(blockstore, wnfs_key));
    }
    ...
    pub fn synced_load_with_wnfs_key(
        ...
        return runtime.block_on(PrivateDirectoryHelper::load_with_wnfs_key(
            blockstore, forest_cid, wnfs_key,
        ));
    }
    ...
    pub fn synced_reload(
        ...
        let runtime = tokio::runtime::Runtime::new().expect("Unable to create a runtime");
        return runtime.block_on(PrivateDirectoryHelper::reload(blockstore, forest_cid));
    }
    ...
    pub fn synced_write_file_stream(
        &mut self,
        path_segments: &[String],
        content: &mut async_std::io::BufReader<async_std::fs::File>,
        modification_time: i64,
    ) -> Result<Cid, String> {
        let runtime = tokio::runtime::Runtime::new().expect("Unable to create a runtime");
        return runtime.block_on(self.write_file_stream(path_segments, content, modification_time));
    }
    ...
    pub fn synced_put(store: &mut FFIFriendlyBlockStore<'a>,key: Vec<u8>, value: Vec<u8>) {
        let runtime = tokio::runtime::Runtime::new().expect("Unable to create a runtime");
        return runtime.block_on(PrivateDirectoryHelper::put(store, key, value));
    }

    pub fn synced_get(store: &mut FFIFriendlyBlockStore<'a>,key: Vec<u8>) {
        let runtime = tokio::runtime::Runtime::new().expect("Unable to create a runtime");
        return runtime.block_on(PrivateDirectoryHelper::get(store, key));
    }
