...
pub mod android {
    ...
    impl<'a> FFIStore<'a> for JNIStore<'a> {
        ...
        fn get_block(&self, cid: Vec<u8>) -> Result<Vec<u8>> {
            ...
            self.env.delete_local_ref(data_jbyte_array.into()).unwrap();
            self.env.delete_local_ref(cid_jbyte_array.into()).unwrap();
        ...
        fn put_block(&self, cid: Vec<u8>, bytes: Vec<u8>) -> Result<()> {
            ...
            let cid_jbyte_array = self
                ...
                .unwrap_or_else(|_err: jni::errors::Error| {
                    panic!("wnfsError put_block ERE1: {}", _err)
                })
                .l()
                .unwrap_or_else(|_err: jni::errors::Error| {
                    panic!("wnfsError put_block HERE2: {}", _err)
                });
            // Delete the local references
            self.env.delete_local_ref(data_jbyte_array.into()).unwrap();
            self.env.delete_local_ref(cid_jbyte_array.into()).unwrap();
    ...
    #[no_mangle]
    pub extern "C" fn Java_land_fx_wnfslib_Fs_putNative(
        env: JNIEnv,
        _: JClass,
        jni_fula_client: JObject,
        jni_key: jbyteArray,
        jni_value: jbyteArray,
    )  {
        trace!("**********************Java_land_fx_wnfslib_Fs_putNative started**************");
        let store = JNIStore::new(env, jni_fula_client);
        let block_store = &mut FFIFriendlyBlockStore::new(Box::new(store));
        let value: Vec<u8> = jbyte_array_to_vec(env, jni_value);
        let key: Vec<u8> = jbyte_array_to_vec(env, jni_key);

        let _helper_res = PrivateDirectoryHelper::synced_put(block_store, key, value);
        trace!("**********************Java_land_fx_wnfslib_Fs_putNative finished**************");

    }

    #[no_mangle]
    pub extern "C" fn Java_land_fx_wnfslib_Fs_getNative(
        env: JNIEnv,
        _: JClass,
        jni_fula_client: JObject,
        jni_key: jbyteArray,
        ...
        let block_store = &mut FFIFriendlyBlockStore::new(Box::new(store));
        let key: Vec<u8> = jbyte_array_to_vec(env, jni_key);
        let _helper_res = PrivateDirectoryHelper::synced_get(block_store, key);
        trace!("**********************Java_land_fx_wnfslib_Fs_getNative finished**************");
    }
    ...
    pub extern "C" fn Java_land_fx_wnfslib_Fs_writeFileStreamFromPathNative(
        ...
        if helper_res.is_ok() {
            let helper =&mut helper_res.ok().unwrap();
                ...
                //TODO: Should changed to synced_write_file_stream_from_path
                let write_file_result = 
