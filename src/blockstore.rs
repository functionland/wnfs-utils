...
pub trait FFIStoreClone<'a> {
    fn clone_box(&self) -> Box<dyn FFIStore<'a>>;
}

impl<'a, T> FFIStoreClone<'a> for T
where
    T: 'static + FFIStore<'a> + Clone,
{
    fn clone_box(&self) -> Box<dyn FFIStore<'a>> {
        Box::new(self.clone())
    }
...
impl<'a> Clone for Box<dyn FFIStore<'a>> {
    fn clone(&self) -> Box<dyn FFIStore<'a>> {
        self.clone_box()
    }
}
...
pub struct FFIFriendlyBlockStore<'a> {
    pub ffi_store: Box<dyn FFIStore<'a>>,
}
